...
export default function Sell() {
  ...
  const myListings = useMemo(() => products.filter((p) => p.seller === (user?.name || 'You') && p.type === 'buy'), [products, user]);
    ...
    <div className="section py-10 space-y-8">
      <div className="card p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="subtle">List your item</p>
            <h2 className="heading">Sell to your campus</h2>
          </div>
        </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-2">Upload multiple images (stays local).</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <textarea
                  ...
                  className="w-full mt-1 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 py-2"
                  rows={3}
                  placeholder="Condition, extras, meet-up details"
                />
              </div>
              <div>
                <label className="text-sm font-semibold">Warranty Card(s) (Opt)</label>
                <div className="flex flex-wrap gap-2 mt-2">
                  {form.warrantyImages.map((img, idx) => (
                    <div key={idx} className="relative w-20 h-20 group">
                      <img src={img} alt={`Warranty ${idx}`} className="w-full h-full object-cover rounded-xl border border-gray-200 dark:border-gray-700" />
                      <button 
                        type="button"
                        onClick={() => set('warrantyImages', form.warrantyImages.filter((_, i) => i !== idx))}
                        className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition shadow"
                      >
                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                      </button>
                  ...
                  <label className="w-20 h-20 flex flex-col items-center justify-center border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        Promise.all(uploaders).then(results => {
                          set('warrantyImages', [...form.warrantyImages, ...results.filter(Boolean)]);
                        });
                      }}
                    />
                  </label>
                </div>
                <p className="text-xs text-gray-500 mt-2">Upload one or more warranty receipts</p>
              </div>
            </div>
          ...
          <div className="card relative overflow-hidden p-4 bg-gradient-to-br from-brand-50 to-white dark:from-gray-900 dark:to-gray-950 border-dashed border-2 border-brand-200 dark:border-brand-800">
            ...
            {form.title ? (
              <div className="relative space-y-2 z-10">
                ...
                <p className="font-bold text-brand-600">₹{form.price || 0}</p>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="heading">Your listings</h3>
        </div>
        {myListings.length === 0 ? (
          <EmptyState title="No listings" subtitle="Add your first item above." />
        ) : (
